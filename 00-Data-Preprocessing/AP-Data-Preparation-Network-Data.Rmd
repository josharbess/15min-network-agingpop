...
There are different classes of footways in the OSM classification system. Check [here](https://wiki.openstreetmap.org/wiki/Key:highway) for details.
...
Plot footways; these are designated footpaths; i.e., mainly/exclusively for pedestrians. This includes walking tracks and gravel paths:
```{r eval=FALSE}
ggplot(hamilton_road %>% 
         filter(str_detect(highway, "footway"))) + 
  geom_sf(aes(color = highway))
```
...
```{r eval=FALSE}
ggplot(hamilton_road %>% 
         filter(str_detect(highway, "motorway|primary|secondary|tertiary|residential|footway"))) + 
  geom_sf(aes(color = highway))
```
...
hamilton_road <- hamilton_road %>% 
  filter(str_detect(highway,
                    "motorway|primary|secondary|tertiary|residential|footway"))
```
...
Link to GRASS GIS for cleaning the topology of the network (GRASS needs to be installed for this to work; see [here](https://grass.osgeo.org/download/)) and also [geocompr](https://geocompr.robinlovelace.net/gis.html#rgrass).
...
Define projection, extent, and resolution:
```{r echo=FALSE, message=FALSE, results='hide'}
execGRASS("g.proj", 
          flags = c("c", "quiet"), 
          proj4 = st_crs(hamilton_road)$proj4string)
b_box = st_bbox(hamilton_road) 
