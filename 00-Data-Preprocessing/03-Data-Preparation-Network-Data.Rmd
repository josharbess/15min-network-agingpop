...
```{r}
hamilton_neighbourhoods <- st_read("Neighbourhood_Boundaries.shp")
```

Project object:
```{r}
hamilton_neighbourhoods <- hamilton_neighbourhoods %>%
...
hamilton_bb <- hamilton_neighbourhoods %>%
  st_transform(4326) %>%
  st_bbox()
```
<!---
Will need to select neighbourhoods first and then define a smaller bounding box, as the entire city will be far too big.
---!>
...
Retreive destination data:
```{r}
destination_osm <- osmdata::opq(bbox =  hamilton_bb, 
                             timeout = 120) %>% 
  osmdata::add_osm_feature(key = 'highway') %>%
...
Save file:
```{r eval = FALSE}
save(destination_osm, 
     file ="destination_osm.RData")
```
